...
export const createEvent =
  (name, category, description, location, city, date, userId) =>
  async (dispatch) => {
...
export const updateEvent =
  (name, category, description, location, city, date, userId, id) =>
  async (dispatch) => {
    const when = date.toString();
    ...
    case ADD_EVENT_RESERVES:
      action.events.reserve.forEach((event) => {
        ...
        if (!newState[event["event_id"]].reserved) {
          newState[event["event_id"]].reserved =
             {users: [event.name], userId: [event.user_id]};
        } else if(!!newState[event["event_id"]].reserved){
          newState[event['event_id']].reserved['userId'].push(event.user_id)
          newState[event['event_id']].reserved['users'].push(event.name)
        }
      });
      return newState;
    case CLEAR_EVENTS:
